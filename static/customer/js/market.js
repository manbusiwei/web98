...
function autotrust(_this,type){
    if(type == 'sell'){
        $('#buy-price').val($(_this).children().eq(2).html()).css({
            'color':'#222',
            'font-size':'14px'
        });
    }
    if(type == 'buy'){
        $('#sell-price').val($(_this).children().eq(2).html()).css({
            'color':'#222',
            'fontSize':'14px'
        });
    }
...
$(function(){
	$('.coin-name').append(coinShortName.toLocaleUpperCase()+'/'+allMarkets[localStorage.getItem('coinType')].toUpperCase());//更新页面中币种简称
...
function tradesWs() {
    ...
    wsTrades.onmessage=function(result){
        ...
        var d=_obj.data;
        if (typeof BP == 'undefined'){
            if(d['S'].length > 0)$('#buy-price').val(to_num(d['S'][0][0]));
            if(d['B'].length > 0)$('#sell-price').val(to_num(d['B'][0][0]));
            BP=1;
        }
        ...
        for (var i = trades.sell.sn - 1; i >= 0; i--) {
            ...
            trades.sell.html += '<dd  onclick="autotrust(this,\'sell\')"> <div  class="sidebar_bg_red" style="width: '
                                + pt + '%"></div> <div class="w_33t red">' + lang('卖') + (i + 1) + '</div> <div class="w_33p is_num"  >'
                                + d['S'][i][0] + '</div> <div class="w_33p is_num" data-point="'+pointData(coinShortName,'numberPre')+'">' + d['S'][i][1] + '</div></div> <div style="display: none;" class="w_80">'
                                + formatfloat(d['S'][i][1]*d['S'][i][0], 4, 0) + '</div> </dd>';
        }
        var Buy_num = 0;
        $('#orderbook-sell').html(trades.sell.html);
        for (var i = 0; i < trades.buy.sn; i++) {
        ...
        for (var i = 0; i < trades.buy.sn; i++) {
            ...
            trades.buy.html += '<dd onclick="autotrust(this,\'buy\')"> <div class="sidebar_bg_green" style="width: '
                                + pt + '%"></div><div class="w_33t green">' + lang('买') + (i + 1) + '</div> <div class="w_33p is_num"  >'
                                + d['B'][i][0] + '</div> <div class="w_33p is_num" data-point="'+pointData(coinShortName,'numberPre')+'">' + d['B'][i][1] + '</div></div> <div class="w_80" style="display: none;">'
                                + formatfloat(d['B'][i][1]*d['B'][i][0], 4, 0) + '</div> </dd>';
        }
        //委托信息
        // $('#orderbook-buy').html(trades.buy.html=trades.buy.html==''?'<p class="no-records">'+lang('暂无记录')+'</p>':trades.buy.html);
        $('#orderbook-buy').html(trades.buy.html);
...
function historyAddHtml(result,api){
    ...
    for (var i = 0; i < len; i++) {
        ...
        historyStr += '<div><span>' + jsonDateFormat(record[1],'hh:mm:ss') + '</span><span class="'+ ups +'">'+type+'</span><span class="is_num">'
                    // + record.fprize + '</span><span>'+getPointNum(currentCoinRate[getLang_rate()+'Para']*record.fprize)+'</span><span class="is_num">'+ record.fcount
                    + record[2] + '</span><span class="is_num" data-point="'+pointData(coinShortName,'numberPre')+'">'+ record[3]
                    +'</span><span style="width:100px" class="is_num">'+record[4]+'</span><div class="clear"></div></div>';
    }
    var TR=d['TR'];
    var ups = TR['INC'] >= 0 ? '+' : '';
    ...
    $('.showDollarOrRmb').html('≈ '+to_num(TR[getLang_rate()])+' '+getLang_rate());//该币种美元或人民币价格
    $('#ticker-vol').html(TR['ODTN'] + ' ' + coinShortName.toUpperCase());//成交量
    $('#ticker-last').html(TR['LDP']);//最新成交价
    $('#ticker-chg').html(ups + TR['INC']+'%');
