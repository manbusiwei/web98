...
function tradesWs() {
    ...
    ws.onmessage=function(res){
        ...
        if(data.status==1){
            var d =data.data;
            var _showType=$('.market_type .active').data('cointype');
            for(var j=0;j<d.length;j++){
                if(_showType==d[j]['trade']['areaId']){
                    renderMarketBox(d[j]);
                }
            }
            goods_num();
...
function renderMarketBox(d){
    ...
    if($('#orderbook-sell'+_tarde['id']).length==0){
        str+='<div class="market_my_entrust clearBoth" id="market_my_entrust'+_tarde['id']+'">'
            ...
            +'<div class="w_55"><span>'+lang('卖/买')+'</span></div><div class="w_95"><span>'+lang('价格')+'</span>(<span>'+_tarde['area']+'</span>)</div>'
            +'<div class="w_100"><span>'+lang('数量')+'</span></div><div class="w_100"><span>'+lang('总计')+'</span></div></dt>'
            +'<div class="listscoll" id="orderbook-sell'+_tarde['id']+'"></div>'
...
function randerTrade(coin,d){
    var _orderbboxS=$('#orderbook-sell'+coin.id)
    var _orderbboxB=$('#orderbook-buy'+coin.id)
    var trades = {sell: {html: '', sn: 1000}, buy: {html: '', sum: 0, sn: 1000}};
    var sort = [], vol = 0;
    var sell_num = 0;
    for (var i = 0; i < trades.sell.sn; i++) {
    ...
    for (var i = trades.sell.sn - 1; i >= 0; i--) {
        ...
        trades.sell.html += '<dd class="'+sell_type_color+'"> <div  class="sidebar_bg_red" style="width: '
                            + pt + '%"></div> <div class="w_55 red">' + lang('卖') + (i + 1) + '</div> <div class="w_95 is_num"  >'
    ...
    var Buy_num = 0;
    _orderbboxS.html(trades.sell.html);
    for (var i = 0; i < trades.buy.sn; i++) {
        if (typeof d.buy[i] == 'undefined') continue;
        Buy_num += parseFloat(d.buy[i][1]);
    }
    $("#orderbook-sell"+coin.id).scrollTop($('#orderbook-sell'+coin.id)[0].scrollHeight);
    vol = 0;
    for (var i = 0; i < trades.buy.sn; i++) {
        if (typeof d.buy[i] == 'undefined') continue;
        ...
        trades.buy.html += '<dd class="'+buy_type_color+'"> <div class="sidebar_bg_green" style="width: '
                            + pt + '%"></div><div class="w_55 green">' + lang('买') + (i + 1) + '</div> <div class="w_95 is_num"  >'
    ...
    _orderbboxB.html(trades.buy.html);
}
